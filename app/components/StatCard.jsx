...
export default function StatCard({ cityName }) {
  ...
  console.log("WEATHER DARA",{weatherData})
  useEffect(() => {
  async function getWeather() {
    ...
    try {
      const { lat, lon, display_name} = await getCoordinates(cityName);
      setDisplayName(display_name);
  
      const data = await fetchForecast(lat, lon);
      setWeatherData(data[0]); //we choose data[0] beacuse its considered the closest one to Cureent time
    } catch (err) {
      console.error(err);
